...
(** Polymorphic Set.
    ...
    This is a polymorphic set, similar to standard library [Set] module but in a
    defunctorized style. *)
...
val add : 'a -> 'a t -> 'a t
(** [add x s] returns a set containing the same elements as [s], plus [x]. If an
    element [y] equal to [x] was already in [s], the resulting set contains [x]
    and not [y]. *)

val remove : 'a -> 'a t -> 'a t
(** [remove x s] returns a set containing the same elements as [s], except for
    [x] which is missing in the returned set. *)

val mem : 'a -> 'a t -> bool
(** [mem x s] returns [true] if [s] contains [x], and [false] otherwise. *)

val iter : ('a -> unit) -> 'a t -> unit
(** [iter f s] applies [f] to all elements in set [s]. The elements are passed
    to [f] in increasing order with respect to the ordering used to create the
    set. *)
...
(** Return the list of all elements of the given set. The returned list is
    sorted in increasing order with respect to the ordering used to create the
    set. *)
