...
(* emacs: do przerabiania outputu toplevela na test 2y M-x query-replace-regexp
   # \(.*\);;[ ]* .* =[ ]*\(.*\) na test 8 ( = );; *)
...
test 66
  (elements (add (-10, 2) s) = [ -10, 4; 6, 7; 9, 10; 14, 15; 17, 18; 20, 21 ])
;;
...
test 81
  (elements (add (19, 25) s) = [ -1, 1; 3, 4; 6, 7; 9, 10; 14, 15; 17, 25 ])
;;
...
test 91
  (elements (add (6, 12) s) = [ -1, 1; 3, 4; 6, 12; 14, 15; 17, 18; 20, 21 ])
;;
...
test 100
  (elements (remove (-10, 1) s) = [ 3, 4; 6, 7; 9, 10; 14, 15; 17, 18; 20, 21 ])
;;

test 101
  (elements (remove (-10, 2) s) = [ 3, 4; 6, 7; 9, 10; 14, 15; 17, 18; 20, 21 ])
;;
...
test 116
  (elements (remove (20, 28) s) = [ -1, 1; 3, 4; 6, 7; 9, 10; 14, 15; 17, 18 ])
;;
...
test 119
  (elements (remove (1, 5) s) = [ -1, 0; 6, 7; 9, 10; 14, 15; 17, 18; 20, 21 ])
;;

test 120
  (elements (remove (1, 6) s) = [ -1, 0; 7, 7; 9, 10; 14, 15; 17, 18; 20, 21 ])
;;
...
test 122
  (elements (remove (2, 5) s) = [ -1, 1; 6, 7; 9, 10; 14, 15; 17, 18; 20, 21 ])
;;

test 123
  (elements (remove (3, 5) s) = [ -1, 1; 6, 7; 9, 10; 14, 15; 17, 18; 20, 21 ])
;;

test 124
  (elements (remove (3, 6) s) = [ -1, 1; 7, 7; 9, 10; 14, 15; 17, 18; 20, 21 ])
;;
...
test 126
  (elements (remove (2, 6) s) = [ -1, 1; 7, 7; 9, 10; 14, 15; 17, 18; 20, 21 ])
;;
...
test 172
  (splituj (-10) s
  = ([], false, [ -1, 1; 3, 4; 6, 7; 9, 10; 14, 15; 17, 18; 20, 21 ]))
;;
...
test 174
  (splituj 0 s
  = ([ -1, -1 ], true, [ 1, 1; 3, 4; 6, 7; 9, 10; 14, 15; 17, 18; 20, 21 ]))
;;
...
test 183
  (splituj 22 s
  = ([ -1, 1; 3, 4; 6, 7; 9, 10; 14, 15; 17, 18; 20, 21 ], false, []))
;;
...
test 253
  (elements (add (min_int, 12) (add (10, max_int) empty)) = [ min_int, max_int ])
...
let dodawaj minim maxim start step s =
  let rec dod a s =
    if minim <= a && a <= maxim
    then dod (a + (2 * step)) (add (min a (a + step), max a (a + step)) s)
    else s
  in
  dod start s

(* Printf.printf "rrr1: %d rrr2: %d rrr3: %d rrr4: %d " (!rrr1) (!rrr2) (!rrr3)
   (!rrr4);; *)
