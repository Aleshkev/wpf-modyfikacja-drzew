...
let simple l =
  let e, res =
    List.fold_left
      (fun ((px, py), la) (x, y) ->
        if py + 1 >= x then (px, max py y), la else (x, y), (px, py) :: la)
      (List.hd l, [])
      (List.tl l)
  in
  List.rev (e :: res)
...
let long l =
  let rec add_inter acc (x, y) =
    if x == y then x :: acc else add_inter (x :: acc) (x + 1, y)
  in
  List.rev (List.fold_left (fun acc inter -> add_inter [] inter @ acc) [] l)

let add_list = List.fold_left (fun s x -> add x s)
let mem_all a l1 = List.filter (fun x -> not (mem x a)) l1 = []
let mem_none a l1 = List.filter (fun x -> mem x a) l1 = []
...
let rec aux l i = if i = 0 then l else aux (i :: l) (i - 1)

let l1 =
  snd
    (List.fold_left
       (fun (i, l) _ -> i + 3, (i, i + 1) :: l)
       (min_int, []) (aux [] 100000))
...
let test s (a, b) step i =
  let rec aux s (x, y) i =
    if i = 0 then s else aux (remove (x, y) s) (x + step, y + step) (i - 1)
  in
  aux s (a, b) i
;;
