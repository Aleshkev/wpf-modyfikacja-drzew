...
let loop f (a, b) s =
  let rec pom a b acc = if a = b then f a acc else pom (a + 1) b (f a acc) in
  pom a b s
...
type testAction = TestAdd | TestRemove | TestSplit | TestBelow

let sort (x, y) = if x < y then x, y else y, x
let interval_to_list (a, b) = List.rev (loop (fun x l -> x :: l) (a, b) [])

let to_int_list ll =
  List.fold_left
    (fun acc el -> List.rev_append (List.rev acc) (interval_to_list el))
    [] ll

let print_intset set = print_list (IntSet.elements set)
let print_iset set = print_list (to_int_list (ISet.elements set))
...
let test_split () : unit =
  let a = random () and side = Random.int 2 in
...
let test_below () : unit =
  ...
  let test = ISet.below a !iset and b, _, _ = IntSet.split (a + 1) !intset in
...
let _ =
  ...
  while !i < n do
    let () =
      if clear_step > 0 && !i mod clear_step = 0
      then (
        Printf.printf "[clear]\n";
        iset := ISet.empty;
        intset := IntSet.empty);
